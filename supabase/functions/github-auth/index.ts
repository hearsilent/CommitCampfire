...
serve(async (req: Request) => {
    ...
    console.log(`[${new Date().toISOString()}] Received ${method} request to ${url}`)
    ...
    try {
        ...
        const client_id = Deno.env.get('GITHUB_CLIENT_ID') || Deno.env.get('VITE_GITHUB_CLIENT_ID')
        const client_secret = Deno.env.get('GITHUB_CLIENT_SECRET')

        if (!client_id || !client_secret) {
            console.error('Missing environment variables')
            return new Response(
                JSON.stringify({ error: 'Internal Server Error', details: 'Server configuration missing.' }),
